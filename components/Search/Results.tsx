...
export default function Results({
  ...
  if (people.length && showResults)
    return (
      <div className="fixed top-0 left-0 md:top-[100px] md:left-6 w-screen md:w-[500px] max-h-[500px] overflow-y-auto bg-white rounded-xl flex flex-col items-center space-x-3 cursor-text p-0">
        {people.map((person, index) => (
          <div
            ...
            className={`flex items-center space-x-3 w-full border-b-2 px-4 py-5 hover:bg-gray-200 transition-all duration-100 cursor-pointer
             ${index === people.length ? "border-none" : ""}`}
